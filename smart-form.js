...
// ==========================================================
// SUCCESS RENDER ENGINE (FINAL STEP UI)
// ==========================================================
(function () {
  ...
  form.addEventListener("submit", function () {
    setTimeout(function () {
      renderSuccess();
    }, 300); // чака Webflow да покаже success-а
  });
  ...
  function renderSuccess() {
    var wrap = document.querySelector(".success-message");
    if (!wrap) return;

    var table = wrap.querySelector(".success-summary-table");
    var preview = wrap.querySelector(".success-cad-preview");

    if (!table || !preview) return;

    var data = {
      chimney: getVal("chimney_exists"),
      water: getVal("water_position"),
      oven: getVal("oven_tall_unit"),
      fridge: getVal("fridge_type"),
      bar: getVal("bar_enabled"),
      wall1: getVal("wall_1"),
      wall2: getVal("wall_2"),
      height: getVal("room_height"),
      countertop: getVal("countertop_finish"),
      plan: getVal("plan"),
      date: getVal("meeting_date"),
      slot: getVal("meeting_slot"),
      custom: getVal("custom_date")
    };
    ...
    function row(label, value) {
      ...
      return `
        <div class="success-row">
          <div class="success-label">${label}</div>
    ...
    table.innerHTML = `
      ${row("Комин", data.chimney)}
      ${row("Вода", data.water)}
      ${row("Фурна", data.oven)}
      ${row("Хладилник", data.fridge)}
      ${row("Бар / остров", data.bar)}
      ${row("Стена 1", data.wall1)}
      ${row("Стена 2", data.wall2)}
      ${row("Височина", data.height)}
      ${row("Плот", data.countertop)}
      ${row("План", data.plan)}
      ${row("Дата", data.custom ? data.custom : data.date)}
      ${row("Час", data.slot)}
    `;

    // =========================
    // ВЗИМАМЕ АКТИВНАТА CAD
    // =========================
    var activeKitchen = document.querySelector(".cad-img-kitchen[style*='block']");
    var base = document.querySelector(".cad-img-base");

    var src = "";

    if (activeKitchen) src = activeKitchen.src;
    else if (base) src = base.src;

    if (src) {
      preview.innerHTML = `<img src="${src}" class="success-preview-img">`;
    }
  }
})();
